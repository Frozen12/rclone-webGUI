document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- State Variables ---
    let isRcloneTransferInProgress = false;
    ...
    stopRcloneTransferBtn.addEventListener('click', () => stopRcloneTransfer(false));
    ...
    stopTerminalBtn.addEventListener('click', () => stopTerminalProcess(false));
    ...
    function startRcloneTransfer() {
        ...
        isRcloneTransferInProgress = true;
            ...
            body: JSON.stringify(payload)
             ...
             function push() {
                 reader.read().then(({ done, value }) => {
                     if (done || !isRcloneTransferInProgress) {
                        // Stop reading if the stream is done or if the user manually stopped it.
                        if (isRcloneTransferInProgress) {
                           // This block runs if the process finished naturally
                             ...
                             try {
                                const data = JSON.parse(dataStr);
                                 if (data.status === 'progress') {
                                     ...
                                     rcloneLiveOutput.classList.add(data.status === 'complete' ? 'success' : 'error');
                                     stopRcloneTransfer(true); // Process finished, stop listening
                                 }
                                 rcloneLiveOutput.scrollTop = rcloneLiveOutput.scrollHeight;
                             } catch(e) {
                                // Ignore empty JSON parsing errors which can happen at the end of a stream
                                if (dataStr.trim()) {
    ...
    async function stopRcloneTransfer(isSilent = false) {
        // Only proceed if a transfer is actually in progress
        if (!isRcloneTransferInProgress) return;
      
        isRcloneTransferInProgress = false; // Immediately set state to stop any ongoing fetch loops
        ...
        // Always reset the UI regardless of backend call success
